...
namespace GF_Operations
{
    public static class GF256
    {
        ...
        public static uint Multiply(uint a, uint b, uint m)
        {
            ...
            int i = 0;
            Console.WriteLine($"a(x) * 1:\t{Convert.ToString(a, 2).PadLeft(8, '0')}");
            while (a != 0 && b != 0) // пока есть что и на что умножать
            {
                if ((b & 1) != 0)
                {
                    Console.WriteLine($"a * x^{i}:\t{Convert.ToString(a, 2).PadLeft(8, '0')}");
                    // если в многочлене b есть член '1' , то "добавляем" (^ -- операция сложения для поля Галуа) весь многочлен a к в аккумулятор единожды
